...
/**
 * Get total like count for a recipe (all users)
 */
export async function getLikeCount(recipeId) {
    try {
...
/**
 * Get total bookmark count for a recipe (all users)
 */
export async function getBookmarkCount(recipeId) {
    try {
        const { count, error } = await supabase
            .from('saved_recipes')
            .select('*', { count: 'exact', head: true })
