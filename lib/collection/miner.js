...
async function extractFromOfficialSite(url) {
  ...
  if (result.menus.length === 0) {
    ...
    try {
      ...
      const html = await response.text();
      const $ = cheerio.load(html);
      const ruleBasedMenus = extractMenusRuleBased($);
      console.log(
  ...
  if (result.menus.length === 0) {
    try {
      ...
      if (menuLink) {
        ...
        const sub$ = cheerio.load(subHtml);
        const subMenus = extractMenusRuleBased(sub$);
 ...
 */
function extractMenusRuleBased($) {
  const menus = [];
  ...
  $("li, div, p, td, dt, dd").each((i, el) => {
    ...
    if (priceMatch && (text.includes("円") || text.includes("¥"))) {
      ...
      if (
        ...
        // First-Principles Allergen Inference
        ...
        menus.push({
          ...
          allergens_contained: inference.contained, // Currently tracking 'contained'. Safe items are inferred by absence/reason.
          safe_from_allergens: inference.safe_from, // Add explicit safety tags
        });
        seenNames.add(name);
      }
    }
  ...
  // Fallback 2: Look for list items near "Menu" headers if price missing
  if (menus.length === 0) {
    $("h2, h3").each((i, header) => {
      if (
        $(header)
          .text()
          .match(/Menu|メニュー|商品/i)
      ) {
        $(header)
          .nextAll("p, ul, div")
          .slice(0, 5)
          .each((j, content) => {
            ...
            if (t.length > 3 && t.length < 30 && !seenNames.has(t)) {
              // Inference for header-based items too
              const inference = inferAllergensFromIngredients(t);

              menus.push({
                name: t,
                price: 0,
                description: "メニュー候補: " + inference.reason,
  ...
  // Fallback 3: Image Alt Text (First Principles: Images convey the menu)
  if (menus.length === 0) {
    $("img").each((i, el) => {
      const alt = $(el).attr("alt");
      let src = $(el).attr("src");

      if (alt && alt.length > 3 && alt.length < 40) {
        // Filter out obvious non-menu items
        if (
          !alt.match(
            /logo|icon|btn|button|arrow|banner|map|spacer|link|tel|mail/i,
          )
        ) {
          if (!seenNames.has(alt)) {
            ...
            if (src && !src.startsWith("http")) {
              try {
                src = new URL(src, "https://example.com").href.replace(
                  "https://example.com",
                  "",
                );
                // Wait, we don't have base URL here easily passed.
                // Ideally we should pass 'url' (sourceUrl) to this function.
                // But for now, we just store what we have. Frontend can handle relative? No.
                // Let's assume most are fine or we accept broken images for V1 self-correction.
                // Actually, 'extractMenusRuleBased' doesn't take 'url' arg.
                // I'll leave it raw for now, or assume absolute if possible.
                // Better: Fix function signature to take 'imgBaseUrl' if needed, but risky refactor.
                // Let's just grab 'src'.
              } catch (e) {}
            }
            ...
            menus.push({
              ...
              allergens_contained: inference.contained,
              safe_from_allergens: inference.safe_from,
              source_image_url: src, // Capture Source
            });
            seenNames.add(alt);
          }
        }
  ...
  try {
    ...
    for (const photoRef of photoRefs.slice(0, 10)) {
      ...
      try {
        ...
        const prompt = `
                この画像を分析してください。メニュー表、料理、アレルギー表の情報を抽出してください。
                ...
                メニュー表がない場合でも、**料理の写真であれば必ず抽出してください**（名前は「料理写真」、価格は0でOK）。
                人物や風景など、明らかに食事と関係ない場合のみ空配列 [] を返してください。
                JSON のみ出力してください。
                `;
