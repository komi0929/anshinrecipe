      ...
      for (const link of uniqueMenuLinks) {
        ...
        try {
          ...
          if (subResponse.ok) {
            ...
            // Also check for images on subpages
            $sub("img").each((i, el) => {
              const src = $sub(el).attr("src");
              if (src && (src.includes("menu") || src.includes("food"))) {
                try {
                  const fullUrl = src.startsWith("http")
                    ? src
...
async function fetchPlaceDetails(placeId) {
  ...
  try {
    ...
    const params = new URLSearchParams({
      fields:
        "nationalPhoneNumber,websiteUri,regularOpeningHours,accessibilityOptions,parkingOptions,paymentOptions,generativeSummary,editorialSummary",
      key: API_KEY,
      languageCode: "ja",
    });
