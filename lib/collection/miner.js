...
async function extractFromOfficialSite(url) {
  ...
  try {
    ...
    const detected = detectFeaturesFromText(pageText);
    result.features = { ...result.features, ...detected };
  } catch (error) {
...
/**
 * Pure function to detect features from text
 */
export function detectFeaturesFromText(pageText) {
  ...
  if (pageText.includes("除去") && pageText.includes("対応")) {
      const negativePatterns = /除去.*(できません|不可|お断り|難しい|致しかね|対応して(おりません|いない)|行って(おりません|いない))/;
