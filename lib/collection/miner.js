...
export async function deepDiveCandidate(candidate) {
  ...
  const placeDetails = await fetchPlaceDetails(candidate.place_id, log);
  if (placeDetails) {
    log(`Fetched Google Place Details: ${placeDetails.phone || "No Phone"}`);
  ...
  let siteData = null; // Declared at function scope for fallback access
  ...
  if (targetUrl) {
    ...
    siteData = await extractFromOfficialSite(targetUrl);
  ...
  // 4. Generate overview if still missing (or if we want to Enhance it with reviews even if site text was poor)
  if (!results.overview && GEMINI_KEY) {
    // If we have reviews but no site text, we can still generate a great overview
    const reviews = placeDetails?.reviews || [];
    if (results.features || reviews.length > 0) {
      results.overview = await generateOverviewFromFeaturesAndReviews(
        candidate.name,
        results.features,
        reviews,
      );
  ...
  if (!results.overview) {
    // Collect metadata from site search if available
    const siteMeta = siteData?.metaDescription || "";
    results.overview = buildFallbackOverview(
      candidate.name,
      results.features,
      siteMeta,
    );
    log("Generated fallback overview (AI failed, used Meta/Features)");
  }
...
function buildFallbackOverview(name, features, metaDescription) {
  // Priority 1: Use SEO Meta Description (usually manual and high quality)
  if (isValidOverview(metaDescription)) {
    console.log(`[Miner] Using meta description for ${name}`);
    return metaDescription;
  }

  // Priority 2: Construct from features (Better than nothing)
  const feats = [];
  if (features.allergen_label === "◯" || features.allergen_label === "△")
    feats.push("アレルギー情報の表示あり");
  if (features.removal === "◯") feats.push("除去食対応可能");
  if (features.gluten_free === "◯") feats.push("グルテンフリーメニューあり");
  if (features.vegan === "◯") feats.push("ヴィーガン対応");
  if (features.kids_menu === "◯" || features.kids_menu === "△")
    feats.push("お子様向けメニューあり");
  if (features.kids_chair === "◯") feats.push("キッズチェア利用可");
  if (features.stroller === "◯") feats.push("ベビーカー入店OK");
  if (features.wheelchair === "◯") feats.push("車椅子でのご来店可");
  if (features.parking === "◯") feats.push("駐車場完備");

  if (feats.length > 0) {
    return `${name}は、${feats.slice(0, 3).join("、")}の飲食店です。詳細なメニューや最新情報は公式サイトまたは店舗へお問い合わせください。`;
  }

  // Priority 3: Minimum viable text
  return `${name}は飲食店です。詳しいメニューや最新情報は公式サイトまたは店舗へお問い合わせください。`;
}

/**
 * Validate if an overview text is acceptable quality
 */
function isValidOverview(text) {
  if (!text || typeof text !== "string") return false;
  const trimmed = text.trim();
  // Must be between 20-300 chars and not contain obvious errors
  if (trimmed.length < 20 || trimmed.length > 300) return false;
  // Reject if it contains AI refusal patterns
  ...
  for (const pattern of rejectPatterns) {
    if (trimmed.toLowerCase().includes(pattern.toLowerCase())) return false;
  }
  return true;
}
 ...
 */
/**
 * Generate store overview using Gemini (Enhanced with Reviews)
 */
async function generateOverview(shopName, pageText, reviews = []) {
  if (!GEMINI_KEY) {
  ...
  try {
    console.log(
      `[Miner] Generating overview for ${shopName} (Text: ${pageText.length} chars, Reviews: ${reviews.length})...`,
    );
    const genAI = new GoogleGenerativeAI(GEMINI_KEY);
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
    ...
    const prompt = `
あなたはプロのグルメライターです。「${shopName}」の魅力を伝える紹介文（150文字程度）を作成してください。
...
【執筆ルール】
1. **お店の「雰囲気」や「体験」を重視**してください（例：「アットホームな」「活気ある」「落ち着いた」など）。
2. レビューから具体的な評判（「天ぷらがサクサク」「提供が早い」など）を抽出し、客観的な事実のように自然に織り交ぜてください。
3. アレルギー対応やキッズ対応の記述があれば必ず含めてください。
4. 「！」などの感嘆符を適度に使用し、読み手に「行ってみたい」と思わせるポジティブなトーンで書いてください。
5. 架空の事実は書かないでください。情報が矛盾する場合はWebサイト情報を優先してください。
6. 具体的な価格やメニュー名は、古い可能性があるため断定を避けてください（「〜が人気」「リーズナブル」などはOK）。
...
`;
    ...
    if (isValidOverview(text)) {
      console.log(
        `[Miner] AI overview generated successfully (${text.length} chars)`,
      );
      return text;
    } else {
      console.log(
        `[Miner] AI overview rejected (invalid: ${text.slice(0, 50)}...)`,
      );
      return "";
    }
  } catch (error) {
...
/**
 * Generate overview from features AND reviews when website text is missing
 */
async function generateOverviewFromFeaturesAndReviews(
  shopName,
  features,
  reviews = [],
) {
  ...
  try {
    ...
    const prompt = `
あなたはプロのグルメライターです。「${shopName}」の紹介文（100〜150文字）を作成してください。
公式サイトのテキストがないため、以下の「特徴リスト」と「利用者の声」を元に構成してください。
...
【執筆ルール】
1. 利用者の声を参考に、お店の雰囲気や人気の理由を推測して記述してください（断定は避ける）。
2. 特徴リストにある設備（駐車場、キッズ対応など）は事実として記載してください。
3. 「利用者の声」がない場合は、特徴リストのみで簡潔に紹介してください。
4. 自然な日本語で、ポジティブな紹介文にしてください。
...
/**
 * Fetch detailed place info from Google Places API (New)
 *
 * DATA SOURCE PRIORITY (重要):
 * ===========================
 * Google Places API から取得できる情報は「確実なデータ」として最優先で使用する。
 *
 * 【Google APIから確実に取得できる情報】
 * - 駐車場 (parkingOptions) ← 構造化データ
 * - 車椅子対応 (accessibilityOptions) ← 構造化データ
 * - 電話番号 (nationalPhoneNumber) ← 構造化データ
 * - 営業時間 (regularOpeningHours) ← 構造化データ
 * - 公式サイト (websiteUri) ← 構造化データ
 *
 * 【Webスクレイピングでのみ取得できる情報】← 補完的にのみ使用
 * - アレルギー対応の詳細（卵・乳・小麦・ナッツ不使用など）
 * - キッズチェア、ベビーカー入店可否
 * - 離乳食持ち込み可否
 *
...
async function fetchPlaceDetails(placeId) {
  ...
  try {
    ...
    const res = await fetch(`${url}?${params}`, {
      headers: {
        "X-Goog-FieldMask":
          "displayName,nationalPhoneNumber,websiteUri,regularOpeningHours,accessibilityOptions,parkingOptions,paymentOptions,editorialSummary,photos,reviews",
      },
    });
    ...
    if (data.reviews) {
      console.log(
        `[Miner] Fetched ${data.reviews.length} reviews from Google Places.`,
      );
    } else {
      console.log(`[Miner] No reviews found in Google Places response.`);
    }
    ...
    const result = {
      ...
      features: {},
      photos: data.photos || [],
      reviews: data.reviews || [], // Capture reviews
    };
...
export function detectFeaturesFromText(pageText) {
  ...
  // Allergen information (general)
  if (
    pageText.match(
      /アレルギー表|アレルゲン一覧|成分表|特定原材料|28品目|グルテンフリー|gluten free|ヴィーガン|vegan|プラントベース/i,
    )
  ) {
  ...
  if (
    pageText.match(
      /卵不使用|卵フリー|卵なし|卵を使わ|卵抜き|エッグフリー|egg.?free|ノンエッグ/i,
    )
  ) {
    features.egg_free = "◯";
  }

  // Milk/Dairy (乳) - 4大アレルゲンの1つ
  if (
    pageText.match(
      /乳不使用|乳製品不使用|乳フリー|ミルクフリー|dairy.?free|乳なし|乳を使わ/i,
    )
  ) {
    features.dairy_free = "◯";
  }

  // Wheat/Gluten (小麦) - 4大アレルゲンの1つ
  if (
    pageText.match(
      /小麦不使用|小麦フリー|グルテンフリー|gluten.?free|wheat.?free|米粉/i,
    )
  ) {
    features.gluten_free = "◯";
  }

  // Peanut/Nuts (ナッツ) - 4大アレルゲンの1つ
  if (
    pageText.match(
      /ナッツ不使用|ナッツフリー|nut.?free|落花生不使用|ピーナツ不使用/i,
    )
  ) {
    features.nut_free = "◯";
  }

  // Check for "Removal" support (Vegan/GF is inherently removal/alternative)
  if (
    pageText.match(/除去|対応|ヴィーガン|vegan|グルテンフリー|gluten free/i)
  ) {
  ...
  // Kids features
  if (pageText.match(/キッズチェア|子供用椅子|お子様用椅子|ベビーチェア/)) {
    features.kids_chair = "◯";
  }
  if (
    pageText.match(
      /ベビーカー.*OK|ベビーカー.*可|ベビーカー.*入店|ベビーカー.*そのまま/,
    )
  ) {
    features.stroller = "◯";
  }
  if (pageText.match(/おむつ.*交換|おむつ替え|授乳室/)) {
    features.diaper = "◯";
  }
  if (
    pageText.match(
      /離乳食.*持ち込み|離乳食.*持込|ミルク.*お湯|お子様メニュー|キッズメニュー/,
    )
  ) {
    features.baby_food = "◯";
  }
