...
function normalizeAreaName(area) {
  // 型安全チェック（92件改善: startsWith エラー対策）
  if (!area || typeof area !== "string") {
    console.warn("[Scout] normalizeAreaName: invalid area input:", area);
    return "";
  }
...
function isAddressInArea(address, targetArea) {
  // 型安全チェック
  if (!address || typeof address !== "string" || !targetArea) return false;
  ...
  const cityMatch = cities.some(
    (c) => typeof c === "string" && address.includes(c),
  );
  ...
  if (filtered.length === 0 && candidates.length > 0) {
    ...
    filtered = candidates.filter(
      (c) =>
        ...
        (c.address.includes("日本") || c.address.includes("〒")),
    );
  }
  ...
  if (SKIP_CHAIN_STORES && CHAIN_BRAND_NAMES.length > 0) {
    const noChain = uniqueCandidates.filter(
      (c) =>
        ...
        !CHAIN_BRAND_NAMES.some((brand) => c.name.includes(brand)),
    );
...
function extractSignals(candidate) {
  const signals = [];
  // 型安全チェック
  const nameStr = typeof candidate.name === "string" ? candidate.name : "";
  const editorialStr =
    typeof candidate.editorial_summary === "string"
      ? candidate.editorial_summary
      : "";
  const textToAnalyze = `${nameStr} ${editorialStr}`.toLowerCase();
  ...
  for (const term of highConfidenceTerms) {
    if (typeof term === "string" && nameStr.includes(term)) {
      signals.push({
        type: "name_match",
        keyword: term,
        source: "shop_name",
