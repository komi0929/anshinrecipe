...
export async function scoutArea(area, _options = {}) {
  ...
  // AUDIT FIX (Jan 20): Apply STRICT filtering.
  // Do not accept Google Places results unless they actually contain the keywords in their text.
  // This prevents "noise" results (generic restaurants) from flooding the database.
  const enrichedCandidates = candidates
    .filter((candidate) => {
      // 1. Analyze text for explicit keywords
      const structuralSignals = extractSignals(candidate);

      // 2. Check if it's a Web Signal (already pre-verified by specific site queries)
      const isWebSignal =
        candidate.signal_source && candidate.signal_source.startsWith("web_");

      // 3. Strict Filter: Must have text keywords OR be a Web Signal
      const hasValidSignal = structuralSignals.length > 0 || isWebSignal;

      if (!hasValidSignal) {
        // console.log(`[Scout] Dropped noise: ${candidate.name} (No keywords)`);
        return false;
      }

      // Attach analyzed signals to candidate for the map step
    ...
    .map((candidate) => {
      ...
      // If this candidate came from a specific strategy (e.g. Paradox), add it as a strategy signal
      if (candidate.signal_source) {
        structuralSignals.push({
          // Web signals get "high" confidence, generic search gets "medium" if validated by text
      ...
      return {
        ...
        // Initial reliability score based on signal count (Max 60 for raw discovery)
