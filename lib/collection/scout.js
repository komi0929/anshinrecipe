...
const CSE_API_KEY = process.env.GOOGLE_CSE_API_KEY || process.env.NEXT_PUBLIC_GOOGLE_CSE_API_KEY || API_KEY;
...
export async function scoutArea(area, options = {}) {
    ...
    const allergyResults = await searchPlaces(`${area} アレルギー対応 レストラン`);
    candidates.push(...allergyResults.map(p => ({ ...p, signal_source: 'direct_search' })));

    // Strategy 2: Gluten-free search
    const gfResults = await searchPlaces(`${area} グルテンフリー カフェ`);
    candidates.push(...gfResults.map(p => ({ ...p, signal_source: 'gluten_free_search' })));

    // Strategy 3: Kids menu search
    const kidsResults = await searchPlaces(`${area} キッズメニュー ファミリー`);
    candidates.push(...kidsResults.map(p => ({ ...p, signal_source: 'kids_menu_search' })));
    ...
    for (const paradox of PARADOX_MATRIX) {
        ...
        candidates.push(...paradoxResults.map(p => ({
            ...p,
        ...
        if (CSE_ID && CSE_API_KEY) {
            ...
            candidates.push(...webResults.map(p => ({
                ...p,
    ...
    console.log(`[Scout] Found ${uniqueCandidates.length} unique candidates with signals`);
...
async function searchWebSignalsExec(queries, area) {
    ...
    for (const query of queries) {
        try {
            ...
            if (data.items) {
                for (const item of data.items) {
                    ...
                    if (shopName) {
                        ...
                        console.log(`[Scout] Web Signal found: ${shopName} -> Resolving to Places...`);
            ...
            console.warn(`[Scout] Web Search failed for query "${query}":`, e.message);
        }
    }
    return results;
}
    ...
    try {
        const response = await fetch('https://places.googleapis.com/v1/places:searchText', {
            ...
            headers: {
                ...
                'X-Goog-FieldMask': 'places.id,places.displayName,places.formattedAddress,places.location,places.websiteUri,places.nationalPhoneNumber,places.editorialSummary,places.photos'
            },
            body: JSON.stringify({
                textQuery: query,
                languageCode: 'ja',
...
function extractSignals(candidate) {
    const signals = [];
    const textToAnalyze = `${candidate.name} ${candidate.editorial_summary}`.toLowerCase();
    ...
    const highConfidenceTerms = ['アレルギー', 'グルテンフリー', '米粉', '低アレルゲン'];
