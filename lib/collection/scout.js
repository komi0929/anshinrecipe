 ...
 *
 * CRITICAL FIX (Jan 24): Added location bias and address filtering to ensure
 * results are actually within the specified area.
 */
...
function isAddressInArea(address, targetArea) {
  ...
  return address.includes(normalized) || address.includes(prefName);
}
...
export async function scoutArea(area, _options = {}) {
  ...
  const locationBias = getLocationBias(area);
  ...
  const gfResults = await searchPlaces(
    `${cleanArea} グルテンフリー カフェ`,
    locationBias,
  );
  candidates.push(
  ...
  const kidsResults = await searchPlaces(
    `${cleanArea} キッズメニュー ファミリー`,
    locationBias,
  );
  candidates.push(
  ...
  // Strategy 4: Rice flour specialty
  const riceFlourResults = await searchPlaces(
    `${cleanArea} 米粉 パン ケーキ`,
    locationBias,
  );
  candidates.push(
    ...riceFlourResults.map((p) => ({
      ...p,
      signal_source: "rice_flour_search",
    })),
  );

  // Strategy 5: Vegan/Plant-based
  const veganResults = await searchPlaces(
    `${cleanArea} ヴィーガン ビーガン プラントベース`,
    locationBias,
  );
  candidates.push(
    ...veganResults.map((p) => ({ ...p, signal_source: "vegan_search" })),
  );
  ...
  for (const paradox of PARADOX_MATRIX.slice(0, 3)) {
    ...
    const paradoxResults = await searchPlaces(query, locationBias);
    candidates.push(
  ...
  const areaFilteredCandidates = candidates.filter((candidate) => {
    ...
    if (!inArea) {
      console.log(
        `[Scout] Filtered out (wrong area): ${candidate.name} - ${candidate.address}`,
      );
    }
    return inArea;
  });
  ...
  const enrichedCandidates = areaFilteredCandidates
    .filter((candidate) => {
      ...
      const hasValidSignal = structuralSignals.length > 0 || isWebSignal;
