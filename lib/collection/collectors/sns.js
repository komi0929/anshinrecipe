...
async function searchCSE(query) {
    try {
        ...
        const processedItems = await Promise.all(data.items.map(async (item) => {
            ...
            const needsVision = snippet.match(/メニュー|お品書き|案内|一覧|表示|成分|特定原材料|不使用/i) ||
                item.title.match(/メニュー|お品書き/i);

            if (imageUrl && needsVision) {
                console.log(`[SNS] High relevance detected for ${title}. Calling Vision AI...`);
                ...
                console.log(`[SNS] Skipping Vision AI for ${title} (Low text relevance).`);
            }
...
function extractMenusFromSnippet(text) {
    ...
    if (text.match(/ナッツ不使用|ピーナッツ不使用|くるみ不使用/)) {
        menus.push({ name: "ナッツ不使用メニュー", supportedAllergens: ["ナッツ"], collectedDate: new Date().toISOString() });
    }
    return menus;
}
