...
export async function scrapeOfficialLists(area) {
  ...
  const sourceData = prefSources.find(
    (p) => area.includes(p.name) || area.includes(p.area),
  );
  const targetUrls = sourceData
    ? sourceData.urls
    : [
  ...
  console.log(
    `[OfficialList] Scraping ${targetUrls.length} sources in ${area}...`,
  );
...
function extractMenusFromText(text) {
  ...
  const regex =
    /(米粉|グルテンフリー|卵不使用|乳不使用)(の|使用した)?(パン|パスタ|ケーキ|クッキー|ラーメン|カレー|ランチ)/g;
  let match;
...
function determineAllergens(keyword) {
  if (keyword.includes("米粉") || keyword.includes("グルテンフリー"))
    return ["小麦"];
