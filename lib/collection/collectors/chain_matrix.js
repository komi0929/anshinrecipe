...
export async function collectChainMatrix(area) {
  console.log(
    `[ChainMatrix] Collecting for ${chains.length} major chains in ${area}...`,
  );
  console.log(
    `[ChainMatrix] API_KEY present: ${!!API_KEY}, length: ${API_KEY?.length || 0}`,
  );

  // If no API key, use fallback with master data only
  if (!API_KEY) {
    console.warn(
      "[ChainMatrix] Google Maps API Key missing. Using fallback (master data only).",
    );
  ...
  const EXCLUDED_CHAINS = officialTargets
    .filter((t) => t.status === "active")
    .map((t) => t.brandName);
  const targetChains = chains.filter(
    (c) => !EXCLUDED_CHAINS.includes(c.brandName),
  );

  console.log(
    `[ChainMatrix] Processing ${targetChains.length} chains (Excluded ${chains.length - targetChains.length} official targets)...`,
  );
...
function generateFallbackData(area) {
  console.log(
    `[ChainMatrix Fallback] Generating data for ${area} from ${chains.length} chain masters...`,
  );
    ...
    const response = await fetch(
      "https://places.googleapis.com/v1/places:searchText",
      {
        ...
        headers: {
          ...
          "X-Goog-FieldMask":
            "places.name,places.displayName,places.formattedAddress,places.location,places.photos,places.nationalPhoneNumber,places.regularOpeningHours,places.websiteUri",
        },
        body: JSON.stringify({
          textQuery: query,
          maxResultCount: 20,
