...
export async function analyzeGoogleMapsReviews(area) {
  ...
  console.log(
    `[GoogleMaps] Searching for allergy-friendly places in ${area}...`,
  );
  ...
  return Array.from(uniquePlaces.values()).map((place) => {
    ...
    const locationExtras = {
      ...
      private_room:
        place.accessibilityOptions?.wheelchairAccessibleEntrance || false, // Proxy: wheelchair usually means space
    ...
    return {
      ...
      features: {
        ...
        wheelchair_accessible: locationExtras.wheelchair_accessible
          ? "◯"
          : null,
      },
...
async function searchPlaces(query) {
  try {
    const response = await fetch(
      "https://places.googleapis.com/v1/places:searchText",
      {
        ...
        headers: {
          ...
          "X-Goog-FieldMask":
            "places.name,places.displayName,places.formattedAddress,places.location,places.editorialSummary,places.photos,places.rating,places.nationalPhoneNumber,places.regularOpeningHours,places.websiteUri,places.parkingOptions,places.accessibilityOptions",
        },
        body: JSON.stringify({
          textQuery: query,
          maxResultCount: 20,
    ...
    console.log(
      `[GoogleMaps] Query "${query}" returned ${data.places?.length || 0} results`,
    );
...
function extractMenusFromText(place) {
  ...
  const specificPatterns = [
    ...
    {
      regex: /ナッツ不使用.*?ケーキ|ケーキ.*?ナッツ不使用/i,
      name: "ナッツ不使用ケーキ",
      allergen: "ナッツ",
    },
    {
      regex: /ナッツ不使用.*?パン|パン.*?ナッツ不使用/i,
      name: "ナッツ不使用パン",
      allergen: "ナッツ",
    },
    {
      regex: /ナッツ不使用.*?カレー|カレー.*?ナッツ不使用/i,
      name: "ナッツ不使用カレー",
      allergen: "ナッツ",
    },
    {
      regex: /ナッツ不使用|ピーナッツ不使用|くるみ不使用|アーモンド不使用/i,
      name: "ナッツ不使用対応",
      allergen: "ナッツ",
    },
  ];
...
export async function analyzeGoogleMapsReviewsNearby(lat, lng, radius = 5000) {
  ...
  console.log(
    `[GoogleMaps] Radius searching at ${lat}, ${lng} (r=${radius})...`,
  );
    ...
    const response = await fetch(
      "https://places.googleapis.com/v1/places:searchText",
      {
        ...
        headers: {
          ...
          "X-Goog-Api-Key": API_KEY,
          "X-Goog-FieldMask":
            "places.name,places.displayName,places.formattedAddress,places.location,places.editorialSummary,places.photos,places.rating,places.nationalPhoneNumber,places.regularOpeningHours,places.websiteUri",
        },
        body: JSON.stringify({
          textQuery: query,
          maxResultCount: 20,
