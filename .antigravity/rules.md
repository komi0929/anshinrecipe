...
## 0. Critical Directives (絶対遵守事項) - Updated Jan 17
*   **Rules First Action:** アクションを起こす前は、必ずこの `rules.md` を読み込み、絶対的なルールとして遵守すること。
*   **Product Instructions are Absolute:** プロダクトに関するユーザーの指示（仕様、制約、アレルゲン限定など）は、絶対的なルールとして記載し、未来永劫遵守すること。
*   **Allergen Scope Distinction (重要):**
    *   **Shared Logic / Anshin Recipe:** 全ての食材・アレルゲン（そば、えび、かに等含む）を処理・解析できるようにすること。リミットをかけてはならない。
    *   **Anshin Map (UI/Feature):** マップ機能としてユーザーに提供するアレルゲンフィルターは、**「小麦」「卵」「乳」「ナッツ」の4種類に限定**する。
*   **User Instruction Memory:** ユーザーの指示は「絶対的なルール」として記憶し、毎回のアクション前に矛盾がないか確認すること。
