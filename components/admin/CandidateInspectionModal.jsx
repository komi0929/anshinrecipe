...
export const CandidateInspectionModal = ({ candidate, isOpen, onClose, onApprove, onReject }) => {
    ...
    useEffect(() => {
        if (candidate) {
            ...
            const meta = candidate.sources?.find(s => s.type === 'system_metadata')?.data || {};
    ...
    const renderFeatureGrid = (features, type) => {
        ...
        const LABELS = isAllergy ? {
            allergen_label: 'メニュー表記', contamination: 'コンタミ対策', removal: '除去食対応', chart: 'アレルギー表'
        } : {
            kids_chair: 'キッズチェア', stroller: 'ベビーカー', diaper: 'おむつ交換', baby_food: '離乳食持込'
        };

        return (
            <div className={`p-4 rounded-2xl border ${isAllergy ? 'bg-orange-50/50 border-orange-100' : 'bg-blue-50/50 border-blue-100'}`}>
                <h3 className={`font-bold mb-3 flex items-center gap-2 ${isAllergy ? 'text-orange-800' : 'text-blue-800'}`}>
                    {isAllergy ? <ShieldCheck size={16} /> : <span className="text-xl">👶</span>}
                ...
                <div className="grid grid-cols-2 gap-2">
                    {Object.entries(LABELS).map(([key, label]) => {
                        ...
                        const isActive = isAllergy ? editedData.features?.allergy?.[key] : editedData.features?.child?.[key];
                        const activeClass = isAllergy ? 'border-orange-200 bg-orange-50 text-orange-600' : 'border-blue-200 bg-blue-50 text-blue-600';

                        return (
                            <div key={key} className={`bg-white p-2 rounded-lg border text-center ${isActive ? activeClass : 'border-slate-100 opacity-60'}`}>
            ...
            <div className="bg-white w-full max-w-6xl h-[90vh] rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row">
                ...
                <div className="w-full md:w-1/2 flex flex-col border-r border-slate-100 h-full">
                    <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                        <div>
                            <h2 className="font-black text-lg text-slate-800">候補データの検証</h2>
                            <p className="text-xs text-slate-500">内容を修正し、プレビューで確認してください</p>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-6">
                        ...
                        <section className="space-y-4">
                            <h3 className="text-sm font-bold text-slate-400 uppercase">基本情報</h3>
                            <div className="space-y-3">
                                <div>
                                    <label className="text-xs font-bold text-slate-500">店舗名</label>
                                    <input
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-500">Instagram URL</label>
                                        <input
                        ...
                        {/* Menu Selection */}
                        <section className="space-y-4">
                            <h3 className="text-sm font-bold text-slate-400 uppercase">掲載メニュー選択 ({selectedMenuIndices.length}件)</h3>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-3 p-3 cursor-pointer" onClick={() => {
                                                if (isSelected) {
                                                    setSelectedMenuIndices(prev => prev.filter(i => i !== idx));
                                                } else {
                                                    setSelectedMenuIndices(prev => [...prev, idx]);
                                                }
                                            }}>
                                                <div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ${isSelected ? 'bg-orange-500 border-orange-500 text-white' : 'bg-white border-slate-300'}`}>
                                                    {isSelected && <CheckCircle size={10} />}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex justify-between items-start">
                                                        <span className="text-sm font-bold text-slate-800 truncate">{menu.name}</span>
                                                        <span className="text-xs text-slate-500 shrink-0">¥{menu.price}</span>
                                                    </div>
                                                </div>
                                                <Button
                                                    variant="ghost"
                                            ...
                                            {activeTab === `menu-${idx}` && (
                                                <div className="p-3 border-t border-slate-100 bg-slate-50/50 space-y-3">
                                                    ...
                                                    {/* Allergy Grid */}
                                                    <div>
                                                        <label className="text-[10px] font-bold text-slate-400 block mb-1">アレルゲン設定 (4大)</label>
                                                        <div className="grid grid-cols-2 gap-2 bg-white p-2 rounded border border-slate-100">
                                                            {['wheat', 'egg', 'milk', 'nut'].map(allergen => (
                                                                <div key={allergen} className="flex items-center justify-between text-[10px] border-b border-slate-50 pb-1 last:border-0">
                                                                    <span className="font-bold text-slate-600 w-10">
                                                                        {{ wheat: '小麦', egg: '卵', milk: '乳', nut: 'ナッツ' }[allergen]}
                                                                    </span>
                                                                    <div className="flex gap-2">
                                                                        <label className="flex items-center gap-1 cursor-pointer">
                                                                            <input
                                                                                ...
                                                                                checked={menu.allergens_contained?.includes(allergen)}
                                                                                onChange={(e) => updateMenuAllergy(allergen, 'allergens_contained', e.target.checked)}
                                                                            />
                                                                            <span className="text-rose-500">使用</span>
                                                                        </label>
                                                                        <label className="flex items-center gap-1 cursor-pointer">
                                                                            <input
                                                                                type="checkbox"
                                                                                className="accent-blue-500"
                                                                                checked={menu.allergens_removable?.includes(allergen)}
                                                                                onChange={(e) => updateMenuAllergy(allergen, 'allergens_removable', e.target.checked)}
                                                                            />
                                                                            <span className="text-blue-500">除去可</span>
                                                                        </label>
                                                                    </div>
                                                                </div>
                                                            ))}
                                                    ...
                                                    <div>
                                                        <label className="text-[10px] font-bold text-slate-400 block mb-1">説明文</label>
                                                        <textarea
                        ...
                        {/* Image Selection */}
                        <section>
                            <h3 className="text-sm font-bold text-slate-400 uppercase mb-2">トップ画像</h3>
                            <div className="flex gap-2 overflow-x-auto pb-2">
                                {candidate.sources?.find(s => s.type === 'system_metadata')?.data?.images?.map((img, idx) => (
                                    <div
                                        key={idx}
                    ...
                    <div className="p-4 border-t border-slate-100 bg-white space-y-3">
                        <Button onClick={handleApprove} className="w-full bg-emerald-500 hover:bg-emerald-600 h-12 text-lg shadow-emerald-200">
                            <CheckCircle className="mr-2" />
                            承認して公開
                        </Button>
                        <button onClick={onReject} className="w-full py-3 text-sm font-bold text-slate-400 hover:text-rose-500 flex items-center justify-center gap-2">
                    ...
                    <div className="relative w-[375px] h-[750px] bg-white rounded-[40px] shadow-2xl border-[8px] border-slate-800 overflow-hidden flex flex-col">
                        ...
                        <div className="h-6 bg-slate-800 w-full flex justify-between px-6 items-center">
                            ...
                            <div className="flex gap-1">
                                ...
                                <span className="w-3 h-3 bg-white rounded-full text-white text-[8px] flex items-center justify-center">🔋</span>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto pb-20 bg-white scrollbar-hide">
                            ...
                            <div className="h-56 relative bg-slate-200">
                                ...
                                <div className="absolute bottom-4 left-4 right-4 text-white">
                                    <div className="text-xs font-bold bg-orange-500 w-fit px-2 py-0.5 rounded mb-1">飲食店</div>
                                    <h1 className="text-2xl font-black leading-tight">{editedData.shopName}</h1>
                                </div>
                            </div>
                            ...
                            <div className="p-4 space-y-6">
                                {/* Action Bar Mock */}
                                <div className="flex gap-2">
                                    {editedData.phone && <div className="flex-1 py-2 bg-slate-50 rounded-xl flex items-center justify-center gap-1 text-xs font-bold text-slate-600 border border-slate-100"><Phone size={12} /> 電話</div>}
                                    {editedData.instagram_url && <div className="flex-1 py-2 bg-pink-50 rounded-xl flex items-center justify-center gap-1 text-xs font-bold text-pink-600 border border-pink-100"><Instagram size={12} /> Insta</div>}
                                    {editedData.website_url && <div className="flex-1 py-2 bg-slate-50 rounded-xl flex items-center justify-center gap-1 text-xs font-bold text-slate-600 border border-slate-100"><Globe size={12} /> Web</div>}
                                </div>
                                ...
                                <div>
                                    ...
                                    <MenuList
                                        menus={editedData.menus.filter((_, i) => selectedMenuIndices.includes(i))}
