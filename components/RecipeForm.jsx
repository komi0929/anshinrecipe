...
export const RecipeForm = ({
  ...
  const [freeFromAllergens, setFreeFromAllergens] = useState(
    initialData.freeFromAllergens || [],
  );
  ...
  const { hasDraft, restoreDraft, clearDraft, dismissDraft } = useDraftAutoSave(
    {
      key: "recipe_form",
      data: draftData,
      debounceMs: 3000,
  ...
  const handleRestoreDraft = () => {
    ...
    if (draft) {
      ...
      if (draft.freeFromAllergens)
        setFreeFromAllergens(draft.freeFromAllergens);
  ...
  useEffect(() => {
    if (!isEditMode && selectedChildren.length > 0 && profile?.children) {
      const selectedKids = profile.children.filter((c) =>
  ...
  useEffect(() => {
    const fetchSmartImport = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          const stepsText =
            d.steps && d.steps.length > 0
              ...
              : "";
          const contentToAdd = [materialsText, stepsText]
            .filter(Boolean)
            .join("\n\n");
        ...
        setSmartImportError(
          `うまく情報を取得できませんでした (${error.message})`,
        );
      ...
      if (items[i].type.indexOf("image") !== -1) {
        ...
        try {
          ...
          if (publicUrl) {
            setMemoImages((prev) => [
              ...prev,
              { image_url: publicUrl, isNew: true },
            ]);
          }
        } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!profile?.isPro && selectedChildren.length === 0) {
      alert("お子様を選択してください");
      return;
    }
  ...
  const removeScene = (sceneToRemove) => {
    setSelectedScenes(
      selectedScenes.filter((scene) => scene !== sceneToRemove),
    );
  };
    ...
    <form onSubmit={handleSubmit} className="recipe-form">
      ...
      <div className="form-section">
        ...
        <div className="form-group" id="recipe-form-image-area">
          <label>
            レシピ画像 <span className="required">*</span>
          </label>
          {isFetchingOgp ? (
        ...
        <div className="form-section animate-fade-in-down">
          ...
          {smartImportError ? (
            ...
            <textarea
              value={
                ingredientsAndSteps ||
                (isSmartImporting ? "AIがレシピ情報を解析中です..." : "")
              }
      ...
      {!profile?.isPro &&
        profile?.children &&
        profile.children.some((c) => c.allergens?.length > 0) &&
        freeFromAllergens.length > 0 && (
          <div className="form-section">
      ...
      <div className="form-section">
        ...
        <div className="scene-selection">
          ...
          {selectedScenes
            .filter((scene) => !MEAL_SCENES.includes(scene))
            .map((scene) => (
      ...
      <div className="form-section">
        <label className="section-label">
          公開設定 <span className="required">*</span>
        </label>
        <div className="flex gap-4 mt-2">
          ...
          >
            {" "}
            <Globe size={24} /> <span className="font-bold">公開</span>{" "}
          ...
          >
            {" "}
            <Lock size={24} /> <span className="font-bold">非公開</span>{" "}
