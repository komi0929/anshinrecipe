...
export default function DataCollectionAdminPage() {
  ...
  const fetchCandidates = async () => {
    try {
      ...
      setLogs((prev) => [
        ...prev,
        `[ERROR] 候補データ取得中にエラーが発生しました: ${error.message}`,
      ]);
    }
  };
  ...
  const fetchLiveRestaurants = async () => {
    try {
      ...
      setLogs((prev) => [
        ...prev,
        `[ERROR] 本番データ取得中にエラーが発生しました: ${e.message}`,
      ]);
    }
  };
  ...
  const handleStartCollection = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setLogs((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] 収集・保存完了。承認待ちボックスを確認してください。`,
        ]);
      } else {
  ...
  const approveCandidate = async (candidateId, options = {}) => {
    try {
      ...
      if (data.success) {
        setLogs((prev) => [
          ...prev,
          `[承認] ${options.shopName || "店舗"} を本番登録しました`,
        ]);
        await fetchCandidates();
      ...
      setLogs((prev) => [
        ...prev,
        `[ERROR] 承認処理中にエラーが発生しました: ${e.message}`,
      ]);
    }
  };
  ...
  const rejectCandidate = async (candidateId) => {
    try {
      ...
      setLogs((prev) => [
        ...prev,
        `[ERROR] 却下処理中にエラーが発生しました: ${e.message}`,
      ]);
    }
  };
      ...
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="space-y-4">
            ...
            {(() => {
              ...
              const validCandidates = candidates.filter((c) => {
                ...
                const hasMenuAllergies = c.menus?.some(
                  (m) =>
                    (m.allergens_contained &&
                      m.allergens_contained.length > 0) ||
                    (m.allergens_removable &&
                      m.allergens_removable.length > 0) ||
                    (m.supportedAllergens && m.supportedAllergens.length > 0),
                );
                const hasFeatures =
                  c.features?.allergy &&
                  Object.values(c.features.allergy).some((v) => v);
              ...
              if (validCandidates.length === 0) {
                return (
                  <div className="text-center py-20 text-slate-400 bg-white rounded-2xl border-2 border-dashed border-slate-100">
                    {candidates.length > 0
                      ? "条件に一致する候補はありません（全表示に切り替えてみてください）"
                      : "承認待ちのデータはありません。"}
                  </div>
                );
              }
        ...
        <CandidateInspectionModal
          ...
          onApprove={async (options) => {
            await approveCandidate(inspectingCandidate.id, {
              ...options,
              shopName: inspectingCandidate.shopName,
            });
            ...
            {liveData.map((shop, i) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => deleteLiveData(shop.id)}
                    className="text-xs text-red-500 font-bold bg-red-50 px-3 py-1 rounded hover:bg-red-100"
                  >
                    削除
                  </button>
                </div>
            ...
            {reports.map((report) => (
              ...
              >
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button size="xs" onClick={() => setActiveTab("live")}>
                    本番データを修正
                  </Button>
                </div>
              </div>
...
function CandidateCard({ data, onInspect }) {
  ...
  const meta =
    data.sources?.find((s) => s.type === "system_metadata")?.data || {};
  const images = meta.images || [];
  const mainImage = images.length > 0 ? images[0].url : null;
  const menuCount = data.menus?.length || 0;

  // Check allergy status
  const hasAllergyInfo = data.menus?.some(
    (m) =>
      m.allergens_contained?.length > 0 || m.allergens_removable?.length > 0,
  );
    ...
    <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-100 hover:shadow-md transition-shadow flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-start mb-1">
          ...
          <span
            className={`text-[10px] px-2 py-0.5 rounded-full font-bold border shrink-0 ${isReliable ? "bg-blue-50 text-blue-700 border-blue-100" : "bg-yellow-50 text-yellow-700 border-yellow-100"}`}
          >
