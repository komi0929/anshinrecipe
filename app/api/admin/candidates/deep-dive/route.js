...
export async function POST(request) {
    try {
        ...
        console.log(`[DeepDiveAPI] Starting deep dive for ${candidate.id}:${candidate.shop_name}`);
        ...
        const { data: updated, error: updateError } = await supabase
            ...
            .update({
                ...
                sources: [...(candidate.sources || []), ...deepData.sources_checked.map(s => ({ type: s.type, url: s.url, status: 'checked' }))],
                // Mark as "mined" or something?
            })
            .eq('id', candidateId)
            .select()
            .single();
