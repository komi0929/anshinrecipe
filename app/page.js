...
const RecipeListPage = () => {
  ...
  const [, setShowOnboarding] = useState(true); // Control wizard visibility
  ...
  const {} = useNotifications(user?.id);
  ...
  const filteredRecipes = React.useMemo(() => {
    const filtered = processedRecipes.filter((recipe) => {
      ...
      const matchesSearch =
        recipe.title.includes(deferredSearchTerm) ||
        (recipe.tags &&
          recipe.tags.some((t) => t.includes(deferredSearchTerm))) ||
        (recipe.positiveIngredients &&
          recipe.positiveIngredients.some((pi) =>
            pi.includes(deferredSearchTerm),
          ));
      ...
      if (profile?.children?.length > 0) {
        if (selectedChildId) {
          ...
          if (!recipe.safeFor.some((c) => c.id === selectedChildId))
            return false;
        } else {
      ...
      if (selectedScene) {
        if (!recipe.scenes || !recipe.scenes.includes(selectedScene))
          return false;
      }
      ...
      <div className="min-h-screen flex flex-col bg-gradient-to-b from-orange-50 to-white">
        <div className="flex-1 flex flex-col items-center px-4 py-8 max-w-[480px] mx-auto w-full">
          ...
          <div className="w-full flex justify-center mb-4">
            <Link
              href="/team"
              className="flex items-center gap-1 text-orange-500 font-bold text-sm bg-orange-50 px-4 py-2 rounded-full hover:bg-orange-100 transition-colors shadow-sm"
            >
              <span>👋 だれがやってるの？</span>
            </Link>
          </div>
  ...
  if (
    user &&
    !profileLoading &&
    (!profile?.id || (profile?.children?.length === 0 && !profile?.isPro))
  ) {
    return (
        ...
        {user && (
          <>
            ...
            <div className="flex items-center gap-2 px-4 mb-3 overflow-x-auto pb-1 scrollbar-hide">
              ...
              <button
                onClick={() =>
                  setSortOrder(sortOrder === "likes" ? null : "likes")
                }
              ...
              <button
                onClick={() =>
                  setSortOrder(sortOrder === "saves" ? null : "saves")
                }
            ...
            <div className="content-wrapper pointer-events-auto">
              <div className="relative max-w-md mx-auto group">
                ...
                <div className="relative flex items-center p-1">
                  <Search
                    className="ml-4 text-orange-400 font-bold shrink-0"
                    size={22}
                  />
                  <input
                    type="text"
