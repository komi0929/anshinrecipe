...
export default function AdminPage() {
  ...
  const load = async () => {
    ...
    const { data: reps } = await supabase
      .from("reports")
      .select(
        "*, recipe:recipes!recipe_id(id,title,image_url), reporter:profiles!reporter_id(username)",
      )
      .eq("status", "pending")
    ...
    const { data: acts } = await supabase
      .from("tried_reports")
      .select(
        "*, recipe:recipes!recipe_id(id,title), user:profiles!user_id(username)",
      )
      .order("created_at", { ascending: false })
      .limit(10);
    setActivity(acts || []);
    ...
    const [pendingCandidates, pendingReports, totalShops, todayReviews] =
      await Promise.all([
        supabase
  ...
  const loadContent = async (type = contentType, page = 1) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/content?type=${type}&page=${page}&limit=15`,
        {
  ...
  const deleteContent = async (type, id, title) => {
    if (
      !confirm(
        `「${title || "このアイテム"}」を削除しますか？この操作は取り消せません。`,
      )
    )
      return;
    const token = sessionStorage.getItem("admin_token");
  ...
  const loadAnnouncements = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/announcement?pin=${pin}&active=false`,
      );
      const data = await res.json();
      if (data.success) {
  ...
  const createAnnouncement = async () => {
    ...
    if (
      !confirm(
        `お知らせ「${newAnnouncement.title}」を全ユーザーに通知しますか？`,
      )
    )
      return;
  ...
  const css = {
    ...
    badge: (v) => ({
      ...
      padding: "2px 6px",
      borderRadius: "10px",
      background: v > 0 ? "#238636" : v < 0 ? "#da3633" : "#30363d",
      color: "#fff",
    }),
    section: {
      gridColumn: "span 3",
  ...
  if (!auth) {
    return (
      ...
      >
        <form
          onSubmit={login}
          style={{ ...css.card, width: "320px", textAlign: "center" }}
        >
    ...
    <div style={css.page}>
      <header style={css.header}>
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
          ...
          <button onClick={load} disabled={loading} style={css.btn}>
            {loading ? "..." : "🔄 Refresh"}
          </button>
          <a
          ...
          >
            ...
            <a href="/admin/data-collection" style={{ textDecoration: "none" }}>
              <div
                style={{
                  ...
                  border:
                    (mapStats?.pendingCandidates || 0) > 0
                      ? "2px solid #f97316"
                      : "1px solid #30363d",
                }}
              >
              ...
              >
                ...
                <div
                  style={{
                    ...css.kpiVal,
                    color:
                      (mapStats?.pendingReports || 0) > 0 ? "#ef4444" : "#fff",
                  }}
                >
            ...
            <div style={css.kpi}>
              ...
              <div
                style={{
                  ...css.kpiVal,
                  color: (mapStats?.todayReviews || 0) > 0 ? "#10b981" : "#fff",
                }}
              >
        ...
        <div style={{ padding: "24px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", marginBottom: "16px" }}>
            <button
              ...
              style={{
                ...css.btn,
                background: contentType === "recipes" ? "#238636" : "#21262d",
              }}
            >
              📝 レシピ一覧
            </button>
            <button
              onClick={() => {
                setContentType("tried_reports");
                loadContent("tried_reports");
              }}
              style={{
                ...css.btn,
                background:
                  contentType === "tried_reports" ? "#238636" : "#21262d",
              }}
            >
              ✨ つくれぽ一覧
            </button>
              ...
              <div style={{ maxHeight: "600px", overflowY: "auto" }}>
                {contentType === "recipes"
                  ? contentList.map((r) => (
                      ...
                      >
                        {r.image_url ? (
                          <img src={r.image_url} style={css.img} alt="" />
                        ) : (
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ color: "#8b949e", fontSize: "11px" }}>
                            ...
                            {!r.is_public && (
                              <span
                                style={{ marginLeft: "8px", color: "#f0883e" }}
                              >
                                🔒 非公開
                              </span>
                            )}
                        ...
                        <button
                          onClick={() => deleteContent("recipe", r.id, r.title)}
                          style={{
                            ...css.btn,
                            background: "#da3633",
                            border: "none",
                  ...
                  : contentList.map((r) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => deleteContent("tried_report", r.id)}
                          style={{
                            ...css.btn,
                            background: "#da3633",
                            border: "none",
              ...
              >
                <button
                  ...
                  onClick={() => {
                    ...
                    loadContent(contentType, contentPage - 1);
                  }}
                  style={{
                    ...css.btn,
                    opacity: contentPagination.page <= 1 ? 0.5 : 1,
                  }}
                >
                  ← 前へ
                </button>
                ...
                >
                  {contentPagination.page} / {contentPagination.totalPages} (
                  {contentPagination.total}件)
                </span>
                <button
                  disabled={
                    contentPagination.page >= contentPagination.totalPages
            ...
            {userList.length === 0 ? (
              ...
              <div style={{ maxHeight: "600px", overflowY: "auto" }}>
                {userList.map((u) => (
                  <div
                    key={u.id}
                    style={{ ...css.row, borderBottom: "1px solid #30363d" }}
                  >
                    {u.avatar_url || u.picture_url ? (
                      <img
                        src={u.avatar_url || u.picture_url}
                        style={{ ...css.img, borderRadius: "50%" }}
                        alt=""
                      />
                    ) : (
                      <div style={{ ...css.placeholder, borderRadius: "50%" }}>
                        👤
                      </div>
                    )}
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        >
                          {u.display_name || u.username || "名前なし"}
                        </span>
                        {u.is_banned && (
                          <span
                            style={{
                      ...
                      <div style={{ color: "#8b949e", fontSize: "11px" }}>
                        レシピ: {u.recipeCount}件 | 登録:{" "}
                        {new Date(u.created_at).toLocaleDateString("ja-JP")}
                      </div>
                      {u.ban_reason && (
                        <div style={{ color: "#f85149", fontSize: "11px" }}>
                          理由: {u.ban_reason}
                        </div>
                      )}
                    </div>
      ...
      {activeTab === "dashboard" && (
        <div style={css.main}>
          ...
          {[
            {
              l: "新規登録",
              t: d.newUsers?.today,
              y: d.newUsers?.yesterday,
              i: "👤",
            },
            {
              l: "レシピ",
              t: d.recipes?.today,
              y: d.recipes?.yesterday,
              i: "📝",
            },
            ...
            <div key={k.l} style={css.kpi}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span>{k.i}</span>
                {k.y !== null && (
                  <span style={css.badge((k.t || 0) - (k.y || 0))}>
                    {(k.t || 0) - (k.y || 0) > 0 ? "+" : ""}
                    {(k.t || 0) - (k.y || 0)}
                  </span>
                )}
          ...
          <div style={{ ...css.card, gridColumn: "span 1" }}>
            <div
              style={{ fontWeight: 600, marginBottom: "16px", color: "#fff" }}
            >
              📈 累計
            </div>
            {[
                ...
                <div key={s.l} style={{ marginBottom: "12px" }}>
                  ...
                  >
                    ...
                    <span style={{ color: "#fff" }}>
                      {s.v ?? 0}{" "}
                      {i > 0 && (
                        <span style={{ color: "#8b949e" }}>({cvr}%)</span>
                      )}
                    </span>
                  </div>
                  <div style={css.barWrap}>
                    <div style={css.bar(pct, s.c)} />
                  </div>
                </div>
              );
            })}
                ...
                return (
                  <div key={day.date} style={{ flex: 1, textAlign: "center" }}>
                    ...
                    <div
                      style={{
                        height: `${Math.max(h, 8)}%`,
                        background: i === w.length - 1 ? "#3b82f6" : "#30363d",
              ...
              >
                ...
                <span style={{ color: "#fff" }}>
                  {data?.features?.smartImport?.starts > 0
                    ? Math.round(
                        (data.features.smartImport.successes /
                          data.features.smartImport.starts) *
                          100,
                      )
                    : 0}
                  %
              ...
              {reports.length === 0 ? (
                ...
                reports.map((r) => (
                  <div key={r.id} style={css.row}>
                    <img
                      src={r.recipe?.image_url || "/placeholder.png"}
                      style={css.img}
                    />
                    <div style={{ flex: 1 }}>
                    ...
                    <button onClick={() => dismiss(r.id)} style={css.btn}>
                      却下
                    </button>
                  </div>
                ))
              ...
              {activity.length === 0 ? (
                ...
                activity.map((a) => (
                  <div key={a.id} style={css.row}>
                    {a.image_url ? (
                      <img src={a.image_url} style={css.img} />
                    ) : (
                      <div style={css.placeholder}>📄</div>
                    )}
                    <div style={{ flex: 1 }}>
