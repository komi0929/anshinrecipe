...
export const useProfile = () => {
        ...
        if (mounted) {
          ...
          if (sessionUser) {
            localStorage.setItem(
              "anshin_auth_user",
              JSON.stringify({ id: sessionUser.id, email: sessionUser.email }),
            );
          } else {
  ...
  const updateUserName = async (name) => {
    ...
    try {
      const { error } = await supabase
        .from("profiles")
        .update({ username: name, display_name: name, updated_at: new Date() })
        .eq("id", user.id);
  ...
  const updateAllergens = async (allergens) => {
    ...
    try {
      const { error } = await supabase
        .from("profiles")
        .update({ allergens, updated_at: new Date() })
        .eq("id", user.id);
      if (error) throw error;
      mutateProfile();
      addToast("アレルギー情報を更新しました", "success");
    } catch (error) {
      addToast("更新に失敗しました", "error");
    }
  };
  ...
  const toggleUserRestaurant = async (restaurantId, status) => {
    ...
    const isCurrentlyInStatus = (
      status === "visited" ? visitedRestaurantIds : wishlistRestaurantIds
